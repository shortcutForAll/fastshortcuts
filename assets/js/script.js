...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (platformTabs.length > 0) {
        // Add has-platform-tabs class to body
        document.body.classList.add('has-platform-tabs');
        ...
        platformTabs.forEach(tab => {
            tab.addEventListener('click', function() {
                ...
                const selectedContent = document.getElementById(platform + '-shortcuts');
                if (selectedContent) {
        ...
        // Load saved preference or default to Windows
        const savedPlatform = localStorage.getItem('preferredPlatform') || 'windows';
        const savedTab = document.querySelector(`[data-platform="${savedPlatform}"]`);
        if (savedTab) {
            savedTab.click();
        } else {
